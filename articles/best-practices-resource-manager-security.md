...
虽然出站方向的通信允许连接到 Internet，但默认情况下，入站方向的通信在连接到 Internet 时会被阻止。默认规则允许 Azure 负载均衡器查看 VM 的运行状况。如果 NSG 节点下的 VM 或 VM 集不参与负载平衡集，你可以重写此规则。
...
AZURE_LOADBALANCER | 表示 Azure 基础结构负载均衡器，将转换为 Azure 数据中心 IP，从中进行 Azure 运行状况探测。仅当与 NSG 关联的 VM 或 VM 集参与负载平衡集的情况下，才需要此项。
