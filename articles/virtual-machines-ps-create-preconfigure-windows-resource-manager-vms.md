...
使用入站 NAT 规则可以设置基于资源管理器的虚拟机，允许来自 Internet 的传入流量并将其放入负载均衡集。在这两种情况下，都必须指定负载均衡器实例和其他设置。
...
### 选项 5：为负载均衡集指定 NIC 名称和负载均衡器实例。

若要创建 NIC 并将其添加到负载均衡集的负载均衡器实例，你需要：

- 前面创建的、针对负载均衡流量创建了规则的负载均衡器实例的名称。
- 要分配给 NIC 的负载均衡器实例的后端地址池的索引编号。
