<properties
	pageTitle="使用 CLI 捕获运行 Linux 的虚拟机的镜像"
	description="了解如何捕获运行 Linux 的 Azure 虚拟机 (VM) 的镜像。"
...
Azure 将此模板视为一个镜像并将其存储在你的镜像列表中。这也是你上载和存储任何镜像的地方。有关镜像的详细信息，请参阅[关于 Azure 中的虚拟机镜像][]。
...
3. 运行以下命令来捕获你的 VM 镜像：
    ...
    若要捕获镜像，必须删除 VM。可以使用 `--deleted` 或 `-t` 来执行此操作。



4. 可以通过检查你的 VM 镜像列表来确认是否已捕获了你的镜像：
    ...
    `| grep <target-image-name>` 部分是可选的，但可以帮助你更轻松地在列表中查找镜像。
