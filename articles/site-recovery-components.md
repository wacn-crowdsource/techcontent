...
**主目标服务器/故障回复主目标服务器** | <p>使用在 Azure 存储帐户中的 blob 存储上创建的附加 VHD，保留从受保护的计算机复制的数据</p><p>故障回复主目标服务器：保留来自 Azure 中已进行过故障转移的虚拟机的复制数据。数据保留在数据存储区中创建的 VMDK 中，数据存储区是在故障回复启用反向复制时选择的。</p> | <p>作为基于 Windows Server 2012 R2 库镜像（用于保护 Windows 计算机）的 Azure 虚拟机或 Windows 服务器进行安装，或作为基于 OpenLogic CentOS 6.6 库镜像（用于保护 Linux 计算机）的 Linux 服务器进行安装</p><p>有两个大小调整选项 – 标准 A3 和标准 D14</p><p>故障回复主目标服务器：在 VMware 虚拟机上运行。其在对计算机进行故障回复的同一主机上进行设置。</p>| <p>在您设置本地物理服务器或 VMware 虚拟机与 Azure 之间的保护时使用。</p><p>故障回复主目标服务器：在以虚拟方式从 Azure 向本地进行故障回复时使用。</p>
