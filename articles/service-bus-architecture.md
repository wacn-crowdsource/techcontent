...
当客户端向服务总线发送请求时，Azure 负载均衡器将其路由到任何一个网关节点。如果请求是现有通知中心的设备注册，网关节点会将注册写入注册存储，并向调用设备发送回复。如果请求是通知消息，网关节点将消息排入通知队列。其中一个通知节点将在通知队列中取消消息排队，并将该消息发送到注册存储中注册的所有设备。如果某个消息要由许多设备接收，则多个通知节点会参与将消息发送到设备的操作。
